...
INT client_number = 0;
unordered_map<LPWSAOVERLAPPED, INT> Client_dict;
unordered_map<INT, Session*> Session_dict;
...
int main()
{
	...
	while (true)
	{
		...
		Session_dict.try_emplace(client_number, move(client));

		client_number++;
	}
}

Session::Session() : Overlap(), Buffer_recv()
{
	ClearOverlap();
...
Session::Session(INT nid, SOCKET sock)
	: id(nid), Overlap(), Buffer_recv()
{
	Client_dict[&Overlap] = sock;
...
void Session::ProceedSend(char* data, size_t size)
{
	auto wbuffer = new WSABUF;
	auto overlap = new WSAOVERLAPPED;

	auto send_datas = new char[BUF_SIZE];
	wbuffer->buf = send_datas;
	wbuffer->len = BUF_SIZE;

	WSASend(Socket, wbuffer, 1, NULL, 0, overlap, SendRoutine);
}
