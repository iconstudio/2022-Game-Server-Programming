...
void ProcessPacket(char* ptr)
{
	...
	switch (ptr[1])
	{
		case SC_LOGIN_OK:
		{
			auto packet = reinterpret_cast<SC_LOGIN_OK_PACKET*>(ptr);

			g_myid = packet->id;
			avatar.m_x = packet->x;
			avatar.m_y = packet->y;
			string a_name = "P";
			a_name += to_string(g_myid);
		...
		case SC_ADD_OBJECT:
		{
			auto my_packet = reinterpret_cast<SC_ADD_OBJECT_PACKET*>(ptr);
		...
		case SC_MOVE_OBJECT:
		{
			auto my_packet = reinterpret_cast<SC_MOVE_OBJECT_PACKET*>(ptr);
		...
		{
			auto my_packet = reinterpret_cast<SC_REMOVE_OBJECT_PACKET*>(ptr);
